...
This release, v3.1, represents several rounds of improvement over our initial release, and its version now corresponds to the CDM version it represents: CDM 3.1. It has been vetted by ten of our sites - the mapping has been completed and it is successfully being used for inter- and intra- network querying. This release corresponds to the PCORnet CDM v3.1, though it is presently missing the death and PRO tables.
...
* Demographics, v3.1: Similar to the i2b2demodata, with a more granular age tree to support pediatric age queries.
* Diagnoses, v3.1: Includes ICD-10 2015AA and ICD-9 2014AA, generated by Lori Phillips from ontologies on BioPortal.
* Procedures, v3.1: Includes ICD-9 2015AA, ICD-10-PCS 2015AA, Partners' RPDR CMS-DRG tree, Beth Israel's MS-DRG tree (version unknown), and Nathan Wilson's HCPCS tree
* Encounters, v3.1: Includes PCORnet-specific additions, and a tree of 3-digit zip codes derived from the i2b2 demo data
* Enrollment, v3.1: To allow sites to specify which patients have "complete longitudinal data" for selected date ranges.
* Vitals, v3.1: A simple vitals implementation including height, weight, blood pressure, and smoking status.
* Labs, v3.1: Selected high-priority lab categories (70) and their LOINC codes, based on LOINC v236. LOINC codes suggested by PCORI in the [Lab Common Measures Guidance June 2016](https://github.com/CDMFORUM/CDM-GUIDANCE/blob/master/2016-June-1_PCORnet%20Lab%20Common%20Measures_LOINC.xlsx) are now all included.
* Medications, v3.1beta:  RxNorm codes organized by NDF-RT (VA Drug Class), based on RxNorm's November 2015 release
