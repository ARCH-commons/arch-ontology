...
CREATE OR REPLACE FUNCTION i2b2metadata.runtotalnum()
  RETURNS void AS
$BODY$
DECLARE 
    curRecord RECORD;
    ...
    v_sqlstring text = 'CREATE OR REPLACE VIEW i2b2metadata.pcornet_master_vw AS ';
    v_union text = '';
...
begin
    ...
    for curRecord IN 
        select 'select i2b2metadata.run_all_counts(' || quote_literal('i2b2metadata.'||c_table_name) || ')' as sqltext
             , c_table_name
    ...
    LOOP 
        ...
        execute 'select count(*) from i2b2metadata.'||curRecord.c_table_name||' where c_totalnum is not null and c_totalnum <> 0' into v_numpats;
    ...
    raise info 'At %: [Re-]creating view i2b2metadata.pcornet_master_vw',clock_timestamp();
    execute v_sqlstring;
end; 
$BODY$
