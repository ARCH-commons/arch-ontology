...
--  3a) PostgreSQL: select i2b2metadata.run_all_counts('schema.table_name');
...
-- DISCLAIMER: the code assumes that pcornet_ tables are in i2b2metadata schema, while observation_fact 
...
---------------------------------------------------------------
CREATE OR REPLACE FUNCTION i2b2metadata.pat_count_by_concept(metadatatable character varying)
  RETURNS void AS
$BODY$
declare 
...
BEGIN 
    ...
    create temp table finalCountsByConcept AS
        ...
        left join i2b2demodata.observation_fact o 
...
---------------------------------------------------------------
CREATE OR REPLACE FUNCTION i2b2metadata.pat_count_modifiers(metadatatable text)
  RETURNS void AS
$BODY$
declare 
    -- see all notes listed in i2b2metadata.pat_count_by_concept().  The methodolgy of the code here is the same
    v_sqlstr text;
    v_num integer;
BEGIN 
...
---------------------------------------------------------------
CREATE OR REPLACE FUNCTION i2b2metadata.pat_count_by_provider(metadatatable character varying)
  RETURNS void AS
$BODY$
declare 
    -- see all notes listed in i2b2metadata.pat_count_by_concept().  The methodolgy of the code here is the same
    v_sqlstr nvarchar(4000);
    v_num integer;
BEGIN
...
-----------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION i2b2metadata.pat_visit_counts(tabname character varying)
  RETURNS void AS
$BODY$
declare 
    v_sqlstr text;
...
---------------------------------------------------------------  
CREATE OR REPLACE FUNCTION i2b2metadata.pat_count_in_equal(tabname character varying)
  RETURNS void AS
$BODY$
declare 
    -- see also the notes listed in i2b2metadata.pat_visit_counts().  The methodolgy of the code here is the same unless noted
    v_sqlstr text;
    curRecord RECORD;
    v_num integer;
BEGIN
    ...
    LOOP 
        if exists(select 1 from information_schema.columns 
            ...
            -- unlike the i2b2metadata.pat_visit_counts(), see long note there, this function dynamic query queries
...
-----------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION i2b2metadata.run_all_counts(tablename text)
  RETURNS void AS
$BODY$
declare 
    sqlstr text;
...
BEGIN      
    ...
    perform i2b2metadata.PAT_VISIT_COUNTS(tablename);
    perform i2b2metadata.PAT_COUNT_BY_CONCEPT(tablename);
    perform i2b2metadata.PAT_COUNT_BY_PROVIDER(tablename);
    perform i2b2metadata.PAT_COUNT_IN_EQUAL(tablename);
    perform i2b2metadata.PAT_COUNT_MODIFIERS(tablename);
